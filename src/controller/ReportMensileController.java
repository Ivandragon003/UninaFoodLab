...
public class ReportMensileController {
    ...
    // Calcolo ricette solo per sessioni in presenza CON ricette
    if (!sessioniInPresenzaIds.isEmpty()) {
        ...
        // FILTRA sessioni con ricette valide
        List<Integer> sessioniInPresenzaConRicette = new ArrayList<>();
        
        for (Integer idSessione : sessioniInPresenzaIds) {
            int numRicette = ricettePerSessione.getOrDefault(idSessione, 0);
            
            if (numRicette > 0) {
                sessioniInPresenzaConRicette.add(idSessione);
        ...
        // Aggiorna conteggio sessioni pratiche (solo quelle valide)
        sessioniPratiche = sessioniInPresenzaConRicette.size();
    ...
    // Media calcolata solo su sessioni con ricette
