...
public class GestioneSessioniController {
    ...
    // -------------------------------------------------
    public boolean validaDataSessione(LocalDate dataSelezionata)
        ...
        if (dataSelezionata.isBefore(inizioCorso)) {
            throw new ValidationException(
                    "La sessione non può essere prima della data di inizio corso.\n\n" +
        ...
        if (conflitto) {
            throw new ValidationException(
                    "⚠️ Esiste già una sessione in data " + dataSelezionata + "\n\n" +
                    "Ogni data può ospitare una sola sessione.\n" +
                    "Scegli una data diversa.");
        }

        Sessione ultimaSessione = sessioni.stream()
                .max(Comparator.comparing(s -> s.getDataFineSessione().toLocalDate()))
                .orElse(null);
        ...
        if (ultimaSessione != null) {
            ...
            if (giorniDistanza < giorniMinimi) {
                ...
                throw new ValidationException(String.format(
                        ...
                        "💡 La prossima data valida è: %s",
                        freq.name(), giorniMinimi, dataFineUltima, dataSelezionata,
                        giorniDistanza, giorniMinimi, dataMinimaConsentita));
            }
        }
