...
public class GestioneRicette {
	...
	public void aggiornaRicetta(int id, Ricetta ricetta) throws ValidationException, DataAccessException {
		...
		try {
			ricettaDAO.findById(id).orElseThrow(() -> new ValidationException("Ricetta non trovata"));
	...
	public List<Ricetta> filtraCombinato(String nome, Integer tempoMin, Integer tempoMax, Integer ingredientiMin,
		...
		return tutteRicette.stream().filter(r -> matchNome(r, nome))
				...
				.filter(r -> matchRange(r.getNumeroIngredienti(), ingredientiMin, ingredientiMax)).toList();
	}
