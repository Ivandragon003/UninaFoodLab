...
public class RegistrazioneChefGUI extends VBox {
	...
	private void handleRegistration() {
		...
		boolean anyEmpty = Stream.of(codFiscaleField, nomeField, cognomeField, emailField, usernameField, passwordField,
				confermaPasswordField).anyMatch(f -> f.getText().trim().isEmpty());

		if (anyEmpty) {
			mostraErrore("❌ Compilare tutti i campi obbligatori");
			Stream.of(codFiscaleField, nomeField, cognomeField, emailField, usernameField, passwordField,
					confermaPasswordField).filter(f -> f.getText().trim().isEmpty())
	...
	private void resetAllFieldStyles() {
		Stream.of(codFiscaleField, nomeField, cognomeField, emailField, usernameField, passwordField,
				confermaPasswordField).forEach(StyleHelper::applyNormalState);
	}
	...
	private void setupAutoResetListeners() {
		Stream.concat(Stream.of(codFiscaleField, nomeField, cognomeField, emailField, usernameField, passwordField,
				confermaPasswordField), Stream.of(dataNascitaPicker)).forEach(ctrl -> {
