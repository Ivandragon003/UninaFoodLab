...
public class VisualizzaSessioniGUI {
	...
	public VBox getRoot() {
		...
		avisoSolaVisualizzazioneLabel
				.setFont(javafx.scene.text.Font.font("Roboto", javafx.scene.text.FontWeight.BOLD, 14));
		avisoSolaVisualizzazioneLabel.setStyle("-fx-background-color: #E3F2FD;" + "-fx-text-fill: #1565C0;"
				+ "-fx-padding: 12;" + "-fx-background-radius: 8;" + "-fx-border-color: #90CAF9;"
	...
	private boolean isChefPartecipante() {
		if (gestioneCorsoController == null || corso.getChef() == null)
			return false;
	...
	private boolean isCorsoFinito() {
		return corso.getDataFineCorso() != null && corso.getDataFineCorso().isBefore(java.time.LocalDateTime.now());
	}
	...
	private HBox createFiltriBox() {
		...
		filtriBox.setStyle("-fx-background-color: " + StyleHelper.BG_WHITE + ";" + "-fx-background-radius: 12;"
		...
		filtriBox.getChildren().addAll(filtroLabel, filtroTipo, sep, ricetteLabel, filtroRicette, spacer, resetBtn);
	...
	private void setupSessioniListConElimina() {
		sessioniList.setCellFactory(lv -> new ListCell<Sessione>() {
...

			{
				...
				container
						.setStyle("-fx-background-color: " + StyleHelper.BG_WHITE + ";" + "-fx-background-radius: 10;");
			}
			...
			protected void updateItem(Sessione s, boolean empty) {
				...
				if (empty || s == null) {
					...
					tipoLabel.setStyle("-fx-font-weight: bold; -fx-font-size: 15px; -fx-text-fill: "
							+ StyleHelper.TEXT_BLACK + ";");
					...
					if (s instanceof Online) {
						...
						Label piattaformaLabel = StyleHelper
								.createCaptionLabel("Piattaforma: " + online.getPiattaformaStreaming());
						infoBox.getChildren().add(piattaformaLabel);
					} else if (s instanceof InPresenza) {
						InPresenza ip = (InPresenza) s;

						Label luogoLabel = StyleHelper
								.createCaptionLabel("Luogo: " + ip.getVia() + ", " + ip.getCitta());
					...
					setStyle("-fx-background-color: transparent; " + "-fx-border-color: " + borderColor + "; "
	...
	private void confermaEliminaSessione(Sessione sessione) {
		...
		if (isCorsoFinito()) {
			StyleHelper.showErrorDialog("❌ Corso Terminato",
	...
	private void apriCreaSessioni() {
		...
		try {
			...
			if (corso.getSessioni() != null && !corso.getSessioni().isEmpty()) {
				for (Sessione s : corso.getSessioni()) {
					if (s.getDataInizioSessione() != null) {
						...
						dateSessioni.add(d);
						if (dataUltimaSessione == null || d.isAfter(dataUltimaSessione)) {
							dataUltimaSessione = d;
						}
					}
				}
			}
			...
			if (dataUltimaSessione != null) {
				LocalDate prossimaDataPossibile = switch (corso.getFrequenzaCorso()) {
				case unica -> dataUltimaSessione;
				case giornaliero -> dataUltimaSessione.plusDays(1);
				case ogniDueGiorni -> dataUltimaSessione.plusDays(2);
				case settimanale -> dataUltimaSessione.plusWeeks(1);
				case mensile -> dataUltimaSessione.plusMonths(1);
				};
			...
			if (nuovaSessione != null) {
				...
				if (nuovaSessione instanceof InPresenza ip) {
					ricette = ip.getRicette() != null ? new ArrayList<>(ip.getRicette()) : new ArrayList<>();
				}
	...
	private void applicaFiltri() {
		...
		for (Sessione s : corso.getSessioni()) {
			boolean passaTipoFiltro = tipoFiltro.equals("Tutti") || (tipoFiltro.equals("Online") && s instanceof Online)
			...
			if (!ricetteFiltro.isEmpty()) {
				try {
					...
					int ricetteSessione = (s instanceof InPresenza)
							? (((InPresenza) s).getRicette() != null ? ((InPresenza) s).getRicette().size() : 0)
							: 0;
		...
		numeroSessioniLabel.setText(
				String.format("Sessioni visualizzate: %d di %d", sessioniFiltrate.size(), corso.getSessioni().size()));
	}
