...
public class VisualizzaCorsiGUI {
    ...
    public void setControllers(VisualizzaCorsiController visualizzaController,
                               GestioneCorsoController gestioneCorsoController, ChefController chefController,
    ...
    private TableView<CorsoCucina> createOptimizedTable() {
        ...
        table.getColumns().addAll(idCol, nomeCol, argomentoCol, prezzoCol, sessioniCol, freqCol, postiCol,
                inizioCol, fineCol);
    ...
    private void loadDataAsync() {
        ...
        Task<List<CorsoCucina>> loadTask = new Task<List<CorsoCucina>>() {
            ...
            protected List<CorsoCucina> call() throws Exception {
                ...
                List<CorsoCucina> list = visualizzaController.getCorsiDelChef();
    ...
    private void applicaFiltriLocali(String nome, String argomento, boolean soloChefLoggato) {
        ...
        Task<Void> filterTask = new Task<Void>() {
            ...
            protected Void call() throws Exception {
                if (soloChefLoggato && visualizzaController != null) {
                    ...
                    List<CorsoCucina> corsiFondatore = cachedCorsiChef.stream()
                            .filter(c -> {
                                String cfLoggato = visualizzaController.getChefLoggato().getCodFiscale();
                                return c.getCodfiscaleFondatore() != null &&
                                        c.getCodfiscaleFondatore().equalsIgnoreCase(cfLoggato);
                            })
                            .collect(java.util.stream.Collectors.toList());

                    cachedCorsiChef = corsiFondatore;

                    System.out.println("[DEBUG] Corsi dove sono fondatore: " + cachedCorsiChef.size());
                }
                ...
                Platform.runLater(() -> {
                    ...
                    if (soloChefLoggato && filteredCorsi.isEmpty()) {
                        StyleHelper.showInfoDialog("Nessun Corso Trovato",
                                "Non ci sono corsi di cui sei fondatore.\n\n"
                                        + "Verifica di aver creato dei corsi.");
                    }
                });
            }
        ...
        new Thread(filterTask, "FilterCorsiThread").start();
    }
}
