...
public class CreaCorsoGUI {
	...
	private VBox createMainLayout() {
		...
		contentBox.getChildren().addAll(createInfoSection(), new Separator(), createDateTimeSection(), new Separator(),
		...
		try {
			...
			if (scelto != null) {
				...
				if (!giaSelezionato) {
					...
					updateChefDisplay();
					showInfo("Chef aggiunto", "Chef aggiunto con successo!");
				} else {
					showInfo("Chef già selezionato", "Questo chef è già stato selezionato.");
				}
			}

		} catch (Exception e) {
			showError("Errore", "Errore nella selezione chef: " + e.getMessage());
			e.printStackTrace();
		}
	}
