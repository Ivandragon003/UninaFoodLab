...
public class ChefMenuGUI {
	...
	private HBox createWindowButtons(Stage stage) {
		...
		Button maximizeButton = StyleHelper.createWindowButton("☐", () -> stage.setMaximized(!stage.isMaximized()), "rgba(255,140,0,0.8)");				StyleHelper.PRIMARY_ORANGE);
		...
		try {
			...
			creaGUI.setOnRicettaCreata(nuovaRicetta -> {
				StyleHelper.showSuccessDialog("Successo",
						String.format("✅ Ricetta '%s' creata con successo!", nuovaRicetta.getNome()));
				mostraBenvenutoIniziale();
			});
			creaGUI.setOnAnnulla(this::mostraBenvenutoIniziale);
			showInContentPane(creaGUI.getContent());
		} catch (Exception e) {
	...
	private void safeRun(Runnable action, String name) {
		try {
			...
			StyleHelper.showErrorDialog("Errore", "Errore durante: " + name + "\n" + e.getMessage());
			e.printStackTrace();
		}
	}
}
